...
## Configuration
Some options do not change very often and you may like to set the permanently rather than entering them every time on the command line. To do this they can be preconfigured with using environment variables or in an appsettings.json file in the same folder as the mecon binary. In the case that a setting is configured or provided on the command line more than once there is an order of precedence where an option on the command line will trump all other configuration:
1. appsettings.json
2. Environment variable
3. Command line option
### Command Line Options also Available in Config
